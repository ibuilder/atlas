...
| Applications and screening | **No surface** | The service refuses screening without recorded consent, audits the refusal as CRITICAL, snapshots the criteria at the decision, and routes criminal history to individual assessment rather than an automatic decline. None of it is reachable: the leasing funnel stops at leads, and there is no route from an application to a lease. |
