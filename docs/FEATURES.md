...
| E-sign | **Complete** | The envelope lifecycle - draft, sent, partially signed, completed, declined, voided, expired - behind a provider adapter, with a working built-in provider that captures a typed-name signature and its consent record (typed name, IP, user agent, timestamp, wording shown). The document's SHA-256 is pinned when the envelope is sent and re-checked before completion, so a file swapped underneath an open envelope voids it rather than inheriting somebody's consent. Completion requires *every* signer. Simple electronic signature, not qualified - see ADR-0008. |
