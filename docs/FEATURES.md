...
| OIDC single sign-on | **Complete** | Sign-in runs at `/auth/sso/<provider>` with the callback beside it. Authorization code flow with PKCE. The state is a single-use database row, so a replayed callback - a refreshed tab, a URL out of a proxy log - fails on the missing state rather than establishing a second session. The ID token is *verified*: signature against the published keys, then issuer, audience, expiry and nonce, with `alg: none` or a symmetric algorithm refused outright. The provider is named in the URL rather than looked up from the submitted address, because a sign-in page that reveals which tenant an address belongs to is an enumeration oracle in front of the login form. This row read **Complete** before the routes existed; see the note under the table. |
...
| SCIM 2.0 provisioning | **Complete** | `/scim/v2`, deliberately outside `/api/v1`: everything there authenticates as a person, and a directory authenticates as an integration presenting a token issued to it. The tenant comes from that token and from nothing in the request - a directory that could name its own tenant could deactivate a different company's staff. The token is stored hashed and shown exactly once, so a leaked database does not hand somebody the ability to deactivate a company. A `PATCH active: false` disables the account *and revokes its live sessions* in one operation. `DELETE` deactivates rather than removing the row, because a user id appears on ledger entries, audit events, and approvals. A filter the implementation does not understand is refused rather than ignored: returning everybody to a query meant to match one person is how a sync decides to deactivate the entire company. Errors come back in SCIM's own envelope, since a directory treats anything else as a transport failure and retries it. |
...
| Embeddable enquiry form | **Complete** | An `<iframe>` snippet an operator pastes into their own site, managed at `/admin/embed-forms` behind `integration.manage`. An iframe rather than a script tag deliberately: the form is served from Atlas's origin, so an enquirer types into Atlas and a cross-site scripting flaw on the operator's marketing site cannot read the fields. Atlas's only anonymous write surface, so nothing about the request is trusted - the publishable key names the organization (resolved through `unscoped`, before any tenant context exists), framing is allow-listed per key from `frame-ancestors`, `Origin` is checked on write, and a honeypot plus a signed render token filter automation. A rejected bot gets the same thank-you page a person does, because telling it which control fired is free tuning for the next attempt. Deliberately collects nothing screening-grade: name, contact, and a message, so a cloned marketing page is never a route to income or date of birth. Revocation is terminal, since the snippet lives in a page the operator may no longer control. |
...
| Accounts payable | **Complete** | Reachable from the console at `/admin/bills` and over the API. Bill recording posts the ledger impact in the same call; approval routes on a threshold; disbursement is a separate grant again. Separation of duties is enforced by *identity*, not by role: whoever recorded a bill cannot approve it however senior they are, because fake-vendor fraud needs one person able to do both halves and a role check does not stop that. The bill page states that reason rather than only hiding the button, since a missing control reads as a bug and a stated one reads as a control. An unset approval threshold means everything needs approval — a control that vanishes when its configuration is missing is not a control. The approved total is snapshotted, so a bill that has changed since is flagged as no longer covered by the decision that authorised it. |
| Bank reconciliation | **Complete** | The workspace is at `/admin/reconciliations`, with the same operations over the API. Statement import is idempotent by fingerprint — re-importing a file inserts nothing, while two genuinely identical transactions on one day both survive — and a row whose amount will not parse is rejected by row and value rather than imported as zero, because a zero here is a difference somebody spends an afternoon hunting. Match suggestions carry their reasons, since a ranked list nobody can interrogate is a list nobody should accept. Auto-match takes only what is confident *and* unambiguous: a near-tie stays unmatched on purpose, because guessing between two payments of the same amount on the same day is how a reconciliation quietly stops meaning anything. One ledger line settles once. Sign-off refuses a non-zero difference, an unresolved exception, or a transaction that is neither matched nor deliberately ignored, and those refusals are the whole value of the screen. |
...
| Inspections | **Complete** | Book, perform, and sign off from `/admin/inspections` and over the API. The checklist is copied onto the inspection at the template version used, so a template edited afterwards cannot rewrite what a completed inspection appears to have asked. Sign-off is refused while any item has no finding, and refused while a *failed* item that demands a photo has none — a clean pass needs no evidence, because photographing forty working light switches is how a checklist stops being completed honestly. That refusal deliberately lands at the inspection rather than weeks later at a deposit disposition, since the photo cannot be taken retrospectively. The page asks the service what is outstanding rather than recomputing the rule, so the page and the refusal cannot disagree. Failed items raise work orders idempotently, and re-recording an item overwrites rather than appends so a replayed offline capture leaves one finding per line. |
